...
clock = time.Clock()
display.set_caption("RL-Dino-Ai Finanl Project")
...
class Ptera(Sprite):
    def __init__(self, speed=5, sizex=-1, sizey=-1):
        ...
        self.ptera_height = [height * 0.82, height *
                             0.75, height * 0.60, height * 0.48]
...
class ChromeDino(object):
    ...
    def step(self, action, record=False):  # 0: Do nothing. 1: Jump. 2: Duck
        ...
        if self.gameOver:
            # save the score in a csv
            with open('scores.csv', 'a') as f:
                f.write(str(self.playerDino.score) + '\n')

            self.__init__()

        state = array3d(state)
        if record:
            return torch.from_numpy(pre_processing(state)), np.transpose(cv2.cvtColor(state, cv2.COLOR_RGB2BGR), (1, 0, 2)), reward, not (reward > 0)
        else:
