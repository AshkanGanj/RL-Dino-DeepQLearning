...
def train(opt):
    ...
    while iter < opt.num_iters:
        ...
        print("Iteration: {}/{}, Loss: {:.5f}, Epsilon {:.5f}, Reward: {}, Q_value".format(
            iter + 1,
            opt.num_iters,
            loss,
            epsilon, reward, q_value))
        losses.append(loss.item())
        q_values.append(torch.mean(q_value).item())
        rewards.append(reward)
        epsilons.append(epsilon)

        # save reward, loss, epsilon, qvalue after each gameover in csv file
        if done:
            episode = episode + 1
            print(episode)
            with open("results.csv", "a") as f:
                f.write("{},{},{},{},{}\n".format(episode, sum(losses)/len(losses), sum(
                    q_values)/len(q_values), sum(rewards)/len(rewards), sum(epsilons)/len(epsilons)))
            losses = []
            q_values = []
            rewards = []
            epsilons = []
